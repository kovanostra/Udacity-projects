...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for act in self.all_actions:
            prenodes = PgNode_a(act).prenodes
            if prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(PgNode_a(act))
                [node.children.add(PgNode_a(act)) for node in self.s_levels[level] if node in prenodes]
    ...
    def add_literal_level(self, level):
        ...
        for act in self.all_actions:
            effnodes = PgNode_a(act).effnodes
            self.s_levels[level] = self.s_levels[level].union(effnodes)
            [act.children.union(effnodes) for act in self.a_levels[level - 1]]
            [node.parents.add(PgNode_a(act)) for node in self.s_levels[level] if node in effnodes]
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        eff1 = node_a1.effnodes
        eff2 = node_a2.effnodes

        for effect in eff1:
            for other_effect in eff2:
                if effect.symbol == other_effect.symbol and \
                   effect.is_pos != other_effect.is_pos:
                   return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        nodes = [(node_a1, node_a2), (node_a2, node_a1)]
        ...
        for n in nodes:
            ...
            for effect in eff:
                for precondition in pre:
                    if effect.symbol == precondition.symbol and \
                       effect.is_pos != precondition.is_pos:
                       return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        par1 = node_a1.parents
        par2 = node_a2.parents

        for parent in par1:
            for other_parent in par2:
                if parent.is_mutex(other_parent):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        par1 = node_s1.parents
        par2 = node_s2.parents

        for parent in par1:
            for other_parent in par2:
                if not parent.is_mutex(other_parent):
                    return False
        return True
        ...
        for level in range(len(self.s_levels)):
            for g in self.problem.goal:
                for state in self.s_levels[level]:
                    if g == state.symbol and \
                       state.is_pos == True and \
                       g not in taboo:
                        level_sum += level
                        taboo.append(g)
